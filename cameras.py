...
def project_point_radial2( P,f, c, k, p ):
    """
    Project points from 3d to 2d using camera parameters
    including radial and tangential distortion
...
def load_cameras_from_xml(bpath, subjects=[1,5,6,7,8,9,11]):
  """Loads the cameras of h36m
  Args
...
def project_point_simple(P,f, c):
    """
    Project points from 3d to 2d using camera parameters (focal length and camera center).
    Args
        P: Nx3 points in camera coordinates
        f: (scalar) Camera focal length
        c: 2x1 Camera center
...
def refine_camera_intrinscis(camera, affine_matrix):
    new_camera = deepcopy(camera)
    
    new_camera[3][0] = new_camera[3][0] * affine_matrix[0][0] + affine_matrix[0][2]
    new_camera[3][1] = new_camera[3][1] * affine_matrix[1][1] + affine_matrix[1][2]
    new_camera[2][0] = new_camera[2][0] * affine_matrix[0][0]
    new_camera[2][1] = new_camera[2][1] * affine_matrix[1][1]
