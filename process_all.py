...
def process_bounding_box(data_path):
    mat_data = mat73.loadmat(data_path)
    masks = mat_data['Masks']
...
def process_view(out_dir, subject, action, subaction, camera):
    ...
    with pycdf.CDF(path.join(subj_dir, 'Poses_D3_Angles_mono', base_filename + '.cdf')) as cdf:
        angles_3d = np.array(cdf['Pose'])[0]
        angles_3d = angles_3d.reshape(-1, 26, 3)
    with pycdf.CDF(path.join(subj_dir, 'Poses_D3_Positions', base_filename + '.cdf')) as cdf:
        global_p3d = np.array(cdf['Pose'])
        global_p3d = global_p3d.reshape(poses_3d.shape[1], 32, 3)
    bbox = process_bounding_box(path.join(subj_dir, 'BBox', base_filename + '.mat'))
    ...
    return {
        ...
        'pose/angle': angles_3d[frame_indices],
        'pose/bbox': bbox[frame_indices],
